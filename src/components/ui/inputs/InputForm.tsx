...
export const InputForm = (props: InputProps) => {
    ...
    const { type, inputName, value, labelName, placeholder, linkImage, onChange } = props;
    ...
    return (
        <>
            ...
            <div className="flex items-center border py-2 px-3 rounded-md border-secondary-200 bg-white">
                ...
                <input type={type} name={`${inputName}`} id={`${inputName}_id`} placeholder={placeholder} className="pl-2 w-full outline-none text-[1.1rem] border-none" onChange={onChange} value={value} required/>
            </div>
        </>
    )
}