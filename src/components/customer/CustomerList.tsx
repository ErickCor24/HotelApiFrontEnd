...
export const CustomerList = () => {
    ...
    return (
        <>
            <h2 className="text-2xl text-principal-700 mb-4 font-medium">Customer List</h2>
            <div className="overflow-y-auto w-full h-dvh flex justify-center items-start">
                <table>
                    ...
                    <tbody>
                        {data?.data.map((customer) => (
                            <tr key={customer.idCustomer} className=" border-b border-secondary-200">
                                <td className="px-6 py-4">{customer.idCustomer}</td>
                                <td className="px-6 py-4">{customer.name}</td>
                                <td className="px-6 py-4">{customer.lastName}</td>
                                <td className="px-6 py-4">{customer.email}</td>
                                <td className="px-6 py-4">{customer.ci || "N/A"}</td>
                                <td className="px-6 py-4">{customer.age || "Unknown"}</td>
                                <td className="px-6 py-4"> <Link className="py-2 px-4 bg-principal-600 text-principal-50 rounded" to={`./update-customer/${customer.idCustomer}`}>Edit</Link></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
